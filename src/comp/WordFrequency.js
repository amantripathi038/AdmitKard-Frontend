    ...
    useEffect(() => {
        if (searchInput !== '') {
            const filteredData = wordFrequency.filter((item) => {
                return Object.values(item).join('').toLowerCase().startsWith(searchInput.toLowerCase())
            })
                                ...
                                <div class="container px-5 py-4 flex flex-wrap mx-auto">
                                    <div class="flex md:flex-nowrap mx-auto  flex-wrap justify-center items-end md:justify-start">
                                        <div class="relative sm:w-64 w-40 sm:mr-4 mr-2">
                                            <input type="text" id="footer-field" placeholder="search" onChange={(e) => setSearchInput(e.target.value)} name="footer-field" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:ring-2 focus:bg-transparent focus:ring-indigo-200 focus:border-indigo-500 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" />
                                        </div>
                                        <button class="inline-flex text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">Search</button>
                                    </div>
                                </div>
